...
inline FragmentCommonData FragmentSetupAdd(float4 i_tex, half3 i_eyeVec, half3 i_viewDirForParallax, half4 tangentToWorld[3], half3 i_posWorld)
{
	...
	//clip(alpha - _Cutoff); // Note: MMDLIT_CLIP_CUTOFF() will be processed later.(See also MMDLit_fragForwardBaseInternal / MMDLit_fragForwardAddInternal)
#endif
...
half4 MMDLit_fragForwardBaseInternal (VertexOutputForwardBase i)
{
	...
	MMDLIT_CLIP_CUTOFF(s.alpha, _Cutoff)
#else
	MMDLIT_CLIP_FAST_CUTOFF(s.alpha, _Cutoff)
#endif
